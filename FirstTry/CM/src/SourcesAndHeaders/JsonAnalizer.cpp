...
string JsonAnalizer::updateContext(string context){
    ...
    else
    {
        ...
        {
            i = i-2;
            while (i<context.length() && context[i]!='.' && i!=0)
                i--;
            amountOfRemoveContestToMake--;
...
list<DictionaryData> JsonAnalizer::getDictionary()
{
    ...
    /*
    char arr[jsonCode.length()+1];
	strcpy(arr, jsonCode.c_str());

    */
    char* arr = new char[jsonCode.length() + 1];
    jsonCode.copy(arr, jsonCode.length());
    arr[jsonCode.length()] = '\0';
